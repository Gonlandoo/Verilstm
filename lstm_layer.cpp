    ...
    for(int i=0;i<4*hidden_size;i++)
    {
        for(int j=0;j<4*hidden_size;j++)
        {
            for(int m=0;m<input_size;m++)
            {
                ...
                cout<<"ihl0_weight[i][m]*ihl0_input_T[m][j]:"<<ihl0_weight[i][m]<<"*"<<ihl0_input_T[m][j]<<"="<<ihl0_weight[i][m]*ihl0_input_T[m][j]<<endl;
            }
        }
    }
}
        ...
        for (int j = 0; j < 4*hidden_size; j++)
        {
            for (int k = 0; k < input_size; k++)
            {
                if (k==0)
                {
                    ...
                    cout<<ihl0_sys12[i][j*4*hidden_size+k]<<endl;
                }
                else
                {
...
static void out_pb_set()//公共变量sys2与protoboard连接
{
...
static void encodedWeight_process()
{
    ...
    for (int i = 0; i < 4*hidden_size; i++)
    {
        for (int j = 0; j < input_size; j++)
        {
            pb.add_r1cs_constraint(r1cs_constraint<FieldT>(pb_encode_row[i]*10+pb_encode_column[j], 1, pb_weight_encoded_indexed[i][j]));
        }   
    }
