...
static int ihl0_sys11[4*20][10];
static int ihl0_sys12[4*20][9];
...
void read_params::read_ihl0_input()
{
    ...
    ifstream inFile("data/l0_input_fixed_reshaped.csv", ios::in);
    string lineStr;  
...
{
    ifstream inFile("data/ihl0_weight_fixed.csv", ios::in);
    string lineStr;  
    vector<vector<string>> strArray;  
    int i,j;
    i=0;
...
{
    ifstream inFile("data/ihl0_bias_fixed.csv", ios::in);
    string lineStr;  
    vector<vector<string>> strArray;  
    int i;
    i=0;
...
{
    ifstream inFile("data/l0_output_fixed_reshaped.csv", ios::in);
    string lineStr;  
    vector<vector<string>> strArray;  
    int i,j;
    i=0;
...
int main()
{
    ...
    pb_variable<FieldT> pb_sys11[4*20][10];
    pb_variable<FieldT> pb_sys12[4*20][9];
    ...
    for (int i = 0; i < 4*hidden_size; i++)
    {
        ...
        {
            ihl0_sys11[i][j]=ihl0_weight[i][j]*ihl0_input[i][j];
        }
    }
    ...
    for (int i = 0; i < 4*hidden_size; i++)
    {
        for (int j = 0; j < 9; i++)
        {
            if (j==0)
            {
                ihl0_sys12[i][j]=ihl0_sys11[i][j]+ihl0_sys12[i][j+1];
            }
            else
            {
                ihl0_sys12[i][j]=ihl0_sys11[i][j]+ihl0_sys12[i][j-1];
    ...
    for (int i = 0; i < 4*hidden_size; i++)
    {
        for (int j = 0; j < 4*hidden_size; j++)
        {
            pb.add_r1cs_constraint(r1cs_constraint<FieldT>(pb_weight[i][j], pb_input[i][j], pb_sys1[i][j]));
        }
    }
    ...
    for (int i = 0; i < 4*hidden_size; i++)
    {
        for (int j = 0; j < 9; j++)
        {
            if (j ==0 )
            {
                pb.add_r1cs_constraint(r1cs_constraint<FieldT>(pb_sys11[i][j]+pb_sys11[i][j], 1, pb_sys12[i][j]));
            }
            else
            {
                pb.add_r1cs_constraint(r1cs_constraint<FieldT>(pb_sys12[i][j-1]+pb_sys11[i][j], 1, pb_sys12[i][j]));
            }
        }
    }
    // weight*input+bias
    for (int i = 0; i < 4*hidden_size; i++)
    {
        for (int j = 0; j < 4*hidden_size; i++)
        {
            pb.add_r1cs_constraint(r1cs_constraint<FieldT>(pb_sys12[i][9]+pv_bias[j], 1, pb_out[i][j]));
        }
    }
